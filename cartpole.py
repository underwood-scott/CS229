...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    # calculate sums
    action0 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
    action1 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])
    # choose random action if sums are equal
    if action0 == action1:
        return np.random.randint(2)
    # return action zero if sum is larger, otherwise return action 1
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    if reward == -1:
        mdp_data['reward_counts'][new_state,0] += 1 # update reward

    mdp_data['reward_counts'][new_state,1] += 1 # update that this state was visited
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    # find sum of state/action pairs, divide transition counts by this
    counts = mdp_data['transition_counts']
    sa_counts = np.sum(counts,axis=1)
    # iterate through state, next_state, action pairs
    for state in range(mdp_data['num_states']):
        ...
        for action in range(counts.shape[2]):
            if sa_counts[state,action] != 0:
                for next_state in range(mdp_data['num_states']):
                        mdp_data['transition_probs'][state,next_state,action] = counts[state,next_state,action]/sa_counts[state,action]
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while not converged:
        ...
        new_v = r+gamma*v.dot(p).max(axis=1)

        mdp_data['value'] = new_v

        # check for convergence
        if np.max(np.abs(v-new_v)) < tolerance:
            converged = True
